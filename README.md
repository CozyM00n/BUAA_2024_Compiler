...
进入新作用域（`{}`）时，调用 `pushTable` 函数，新建一个符号表并将其入栈，并更新当前`curTableId`为新建的符号表id。
...
同语义分析一样，中间代码生成也需要遍历语法树并获取符号表相关信息。我没有选择重建符号表，在语义分析阶段`symbolManager`中的`symbolMap`已经存储了符号表的全部信息，因此可以直接使用。
